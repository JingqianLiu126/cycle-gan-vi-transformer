...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netG == 'vitran_6blocks':
        net = ViGenerator(input_channels = 3, out_channels = 3, initialize_size = 32, dim_vitrans = 192, blocks = 6, num_heads = 8, dim_head = None, dropout = 0.)
    elif netG == 'unet_128':
...
class Embeddings(nn.Module):
    # combine patch embeddings with class token and position embeddings
    def __init__(self):
        super().__init__()
        self.patch_embeddings = PatchEmbeddings()
        self.cls_token = nn.Parameter(torch.rand(1,1,192))
        self.position_embeddings = \
            nn.Parameter(torch.rand(1,self.patch_embeddings.num_patches + 1, self.patch_embeddings.hidden_size))
    def forward(self,x):
        ...
        #print("####embedding###")
        #print(self.position_embeddings.size(), x.size())
...
class SineLayer(nn.Module):
    ...
    def forward(self, input):
        #print("SineLayer",self.out_features)
...
class ViGenerator(nn.Module):
    def __init__(self,
        ...
        self.encoder = Encoder(dim_vitrans, blocks, num_heads, dim_head, dropout)
        # Implicit Neural Representation
        self.w_out = nn.Sequential(
            #SineLayer(dim_vitrans, dim_vitrans * 2, is_first = True, omega_0 = 30.),
            #SineLayer(dim_vitrans * 2, self.initialize_size * 8 * self.out_channels, is_first = False, omega_0 = 30)
            SineLayer(dim_vitrans, dim_vitrans, is_first = True, omega_0 = 30.)
        )
        self.sln_norm = SLN(self.dim_vitrans)

    def forward(self, x):
        # calculate the embedding output
        #print("input x",x.size())
        #print(x)
        pos_emb, x = self.embedding(x)
        #print(pos_emb.size(),x.size())
        # calculate the encoder's output
        #tmp = torch.rand(1,64,192).cuda()
        #print("this is my test:", self.sln_norm(self.encoder(tmp,tmp)[0]).size())
        x, hl = self.encoder(pos_emb, x)
        hl, x = hl[:,1:,], x[:,1:,:]
        x = self.sln_norm(hl, x)
        #print('after sln',x.size())
