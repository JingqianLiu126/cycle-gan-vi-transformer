...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = ViGenerator(input_channels = 3, out_channels = 3, initialize_size = 16, dim_vitrans = 384, blocks = 6, num_heads = 8, dim_head = None, dropout = 0.)
    elif netG == 'unet_128':
...
class Embeddings(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.patch_embeddings = PatchEmbeddings(self)
        self.cls_token = nn.Parameter(torch.rand(1,1,384))
        self.position_embeddings = \
            nn.Parmater(torch.rand(1,self.patch_embeddings.num_patches + 1, self.patch_embeddings.hidden_size))
    def forward(self,x):
...
class Encoder(nn.Module):
    def __init__(self, 
        ...
        self.blocks = self.__make_layers(dim, blocks, num_heads, dim_head, dropout)
